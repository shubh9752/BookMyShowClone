...
const EntertainCardSlider = () => {
  const EntertainImage = [
  ...
  return (
    <>
      <Slider {...settings}>
        {EntertainImage.map((eachImage, index) => (
          <EntertainCard src={eachImage} key={index} />
        ))}
      </Slider>
    </>
  );
...
export default EntertainCardSlider;